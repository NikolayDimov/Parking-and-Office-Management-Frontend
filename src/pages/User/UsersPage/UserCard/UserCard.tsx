...
export default function UserCard({ user, deleteUser, userReservations }: UserCardProps) {
    return (
        <StyledUserCard>
            {user.imgUrl ? <UserImage src={user.imgUrl} alt="User" /> : <UserImage src={userImg} alt="Default User" />}
            <UserCardDetails user={user} deleteUser={deleteUser} userReservations={userReservations} />
        </StyledUserCard>
    );
}
